...
var Magnifier = function (evt, options) {
	var gOptions = options || {},
		...
		curData = {
			...
			zoomAttached: false,
			zoomable:
				gOptions.zoomable !== undefined
					? gOptions.zoomable
					: false,
			onthumbenter:
				gOptions.onthumbenter !== undefined
					? gOptions.onthumbenter
					: null,
			onthumbmove:
				gOptions.onthumbmove !== undefined
					? gOptions.onthumbmove
					: null,
			onthumbleave:
				gOptions.onthumbleave !== undefined
		...
		gZoomMin =
			gOptions.zoomMin !== undefined
				? gOptions.zoomMin
				: curData.zoomMin,
		gZoomMax =
			gOptions.zoomMax !== undefined
				? gOptions.zoomMax
				: curData.zoomMax,
			...
			if (curData.zoom >= curData.zoomMax) {
				...
				if (handler !== null) {
					handler({
						...
						y: pos.y,
						zoom:
							Math.round(curData.zoom * multiplier * 10) /
							10,
						w: curData.lensW,
						h: curData.lensH,
					});
	...
	this.set = function (options) {
		...
		data[idx] = {
			...
			largeWrapperW:
				mode === "outside" ? largeWrapper.offsetWidth : null,
			largeWrapperH:
